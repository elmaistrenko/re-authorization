...
const securitySchemes = (headerName = constants.DEFAULT_HTTP_ACCESS_HEADER) => ({
    ApiKeyAuth: {
        type: 'apiKey',
        in: 'header',
        name: headerName,
...
const security = (headerName = constants.DEFAULT_HTTP_ACCESS_HEADER) => Object.keys(securitySchemes(headerName)).map(key => ({[key]: []}));

function authPaths(base, tag, headerName = constants.DEFAULT_HTTP_ACCESS_HEADER) {
    const r = {};
